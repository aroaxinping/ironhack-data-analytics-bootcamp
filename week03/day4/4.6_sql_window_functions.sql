...
-- Window functions = aggregation_functions + other_functions
...
SELECT status,
       ...
       duration, 
       amount,
       AVG(amount) OVER(PARTITION BY status, duration ORDER BY status, duration) AS 'Avg_amount'
FROM bank.loan;
...
SELECT status,
	  ...
	  RANK() OVER(PARTITION BY status, duration ) AS 'Rank',
      DENSE_RANK() OVER(PARTITION BY status, duration ) AS 'Dense Rank',
	  ROW_NUMBER() OVER(PARTITION BY status, duration ) AS 'Row Number'
FROM bank.loan;
...
-- Note: A12 is technically "unemployment rate '95" (not a generic/latest rate --
-- A13 would be '96), per the case study's own column reference. Ranked DESC on
-- purpose (highest unemployment = rank 1), same direction as inhabitants/salary,
-- just so all three columns read "1 = most" consistently.

-- 1. Rank districts by inhabitants, average salary, and unemployment rate --
-- no PARTITION BY, so each RANK() is computed across all 77 districts at once.
...
-- Hl.m. Praha (Prague) is #1 on both inhabitants and avg_salary -- no surprise,
-- it's the capital -- but only #76 out of 77 on unemployment, i.e. nearly the
-- lowest unemployment in the whole country. Being the biggest doesn't mean
-- being the most "at risk" on every variable at once.

-- 2. Same three rankings, but PARTITION BY region -- each RANK() now restarts
-- at 1 within every region instead of running across the whole country.
SELECT A2 AS district_name, A3 AS region, A4 AS inhabitants,
...
-- Only difference from query 1 is PARTITION BY A3 added to every OVER() -- same
-- RANK() logic, just computed separately per region instead of over all 77 rows
-- at once. E.g. Mlada Boleslav isn't the biggest district nationally, but it's
-- #1 for avg_salary within central Bohemia specifically.
...
FROM bank.monthly_active_users;   -- Using the previously created view that aggregates monthly active users
