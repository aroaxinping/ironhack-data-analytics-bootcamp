...
-- "Most active" = highest number of transactions. Two CTEs chained together:
-- 1) transaction count per client (only clients whose OWN district is in Central Bohemia)
-- 2) the max count per district, so we can match it back and keep only the winner(s)
...
-- clients tied on the exact same count -- makes sense: those are joint accounts (OWNER +
-- DISPONENT sharing one account), so both clients rack up identical transaction counts.
-- Not a bug, just what "most active" looks like on a shared account.
...
-- "Withdrawal" = trans.type IN ('VYDAJ', 'VYBER') -- both mean money going out, just two labels
...
-- 1219 clients withdrew something in that final week; top one moved 65,500.
