...
-- 2.
SELECT A2 AS district_name FROM bank.district;   -- 77 rows, confirmed

-- 3. Bonus
SELECT A2 AS district_name, A3 AS region
FROM bank.district
...
-- 2. status 'B' = "contract finished, loan not payed" (see case_study_extended.pdf)
SELECT * FROM bank.loan
WHERE status = 'B';
...
-- 4.
SELECT loan_id, account_id, ROUND(amount - payments, 2) AS debt
FROM bank.loan
WHERE status = 'B';
...
-- 6. rural population > 50%  <=>  urban ratio (A10) < 50%
SELECT A2 AS district_name, ROUND(A4 * A10 / 100, 0) AS urban_population
FROM bank.district
WHERE A10 < 50;
...
SELECT account_id, district_id, frequency, date,
       SUBSTR(CAST(date AS CHAR), 1, 2) AS year
FROM bank.account;
...
-- 2.
SELECT MIN(DATE_FORMAT(CONVERT(SUBSTRING_INDEX(issued, ' ', 1), DATE), '%Y')) AS first_gold_year
FROM bank.card
WHERE type = 'gold';   -- 1995

-- 3.
SELECT
    DATE_FORMAT(CONVERT(SUBSTRING_INDEX(issued, ' ', 1), DATE), '%M %D, %Y') AS date_issued,
...
-- Null/empty values usually come from optional fields that weren't filled in at the time
-- of the record (e.g. k_symbol only applies to certain transaction types), or from how the
-- original data export handled missing values (blank string vs a true NULL).

-- amount is always populated in this dataset -- 0 rows either way
SELECT * FROM bank.trans
WHERE amount IS NULL OR amount = '';

-- one query, two counts side by side
SELECT
    SUM(k_symbol IN ('', ' ')) AS empty_k_symbol,
    SUM(k_symbol NOT IN ('', ' ')) AS non_empty_k_symbol
FROM bank.trans;
...
-- NOTE: this exercise's wording (table `payment`, "rental revenue", `customer_id`) doesn't
-- match this bank case study -- it reads like leftover text from a different generic SQL
-- course dataset (a DVD-rental schema) that wasn't adapted here. There's no `payment` table,
-- and no `customer_id` reachable without a JOIN (a customer connects to an order via
-- disp -> account, and JOINs aren't covered until tomorrow). Adapted both questions to the
-- closest equivalent within today's scope: `order` (recurring debits/revenue) and
-- `account_id` (the level `order` actually groups by without a join).

-- 1.
SELECT ROUND(SUM(amount), 2) AS Total_revenue
FROM bank.order;   -- 21,228,993.60

-- 2.
SELECT account_id, ROUND(SUM(amount), 2) AS Total_revenue
FROM bank.order
GROUP BY account_id
